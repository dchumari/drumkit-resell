...
def process_and_rename_kit(root_dir: str, rebranded_name: str = "Resold", genre: str = "Trap", ai_naming: Optional[bool] = None) -> Tuple[Dict[str, List[str]], List[str]]:
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        # Skip macOS resource-fork metadata folders generated by Mac zip utilities.
        # These contain only ._* sidecar files (Apple Double format) — zero audio content.
        # Real .aif/.aiff audio files in non-__MACOSX folders are unaffected and preserved.
        ...
        for name in filenames:
            # Skip Apple Double resource fork sidecars (e.g. ._snares 1.wav).
            # These are metadata files created by macOS — not listenable audio.
    ...
    for cat_caps, files in allowed_files_by_cat.items():
        ...
        for idx, (old_path, filename, descriptor) in enumerate(files, 1):
            ...
            # cat_dir was already created above, but this guards against
            # any edge case where makedirs didn't complete.
            try:
                os.makedirs(os.path.dirname(new_path), exist_ok=True)
            except Exception as dir_err:
                print(f"Error creating destination directory for {new_name}: {dir_err}")
                continue
